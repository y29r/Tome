...
## Math Operators
Tome has another unique way to mutate objects. And that is with the standard mathematical math operators. Currently Tome supports:
- `+` : Adds an object into the Tome
- `-` : Removes an object from the Tome (doesn't destroy it)

> [!NOTE]
> Internally Tome uses a metatable hooked to the math metamethods, this means you will get more overhead when appending objects. If you prefer adding objects like this, ensure performance isn't an issue. It's important to keep in mind that these operations are atomic `~<0.1μs` so it may not really matter.
...
local part: Part = newTome + workspace.Part
print(newTome:Contains(part)) --> true

local part: Part = newTome - workspace.Part
print(newTome:Contains(part)) --> false
```
